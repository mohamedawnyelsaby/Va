      ...
      <div style={{ padding: 'clamp(2rem,5vw,4rem) clamp(1.5rem,7vw,5rem)', maxWidth: '900px', margin: '0 auto' }}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '6rem 2rem' }}>
            ...
            <p style={{ fontFamily: 'var(--font-dm-sans)', fontSize: '0.86rem', color: 'var(--vg-text-3)', marginBottom: '2rem', lineHeight: 1.7 }}>
          ...
          <>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1px', background: 'var(--vg-border)', marginBottom: '2rem' }}>
              {bookings.map(b => {
                ...
                const { Icon } = cfg;
                return (
                  <div key={b.id} style={{ background: 'var(--vg-bg-card)', padding: '1.5rem clamp(1rem,3vw,1.8rem)', display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: '1.2rem', alignItems: 'center', transition: 'background 0.2s' }}
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                        <span style={{ fontFamily: 'var(--font-space-mono)', fontSize: '0.44rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--vg-text-3)' }}>
                          {new Date(b.startDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                        </span>
                        <span style={{ fontFamily: 'var(--font-space-mono)', fontSize: '0.44rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: cfg.color }}>
                          {cfg.label}
                        </span>
                      </div>
                    </div>
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      ...
                      {b.itemType && <div style={{ fontFamily: 'var(--font-space-mono)', fontSize: '0.42rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--vg-text-3)', marginTop: '0.2rem' }}>{b.itemType}</div>}
                    </div>
                  </div>
                );
              })}
            ...
            {totalPages > 1 && (
              <div style={{ display: 'flex', justifyContent: 'center', gap: '0.5rem' }}>
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => (
                  <button key={i + 1} onClick={() => setPage(i + 1)} style={{ width: '38px', height: '38px', background: page === i + 1 ? 'var(--vg-gold)' : 'none', border: `1px solid ${page === i + 1 ? 'var(--vg-gold)' : 'var(--vg-border)'}`, color: page === i + 1 ? 'var(--vg-bg)' : 'var(--vg-text-2)', fontFamily: 'var(--font-space-mono)', fontSize: '0.5rem', cursor: 'pointer', transition: 'all 0.2s' }}>{i + 1}</button>
                ))}
