  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--vg-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <div style={{ fontFamily: 'var(--font-space-mono)', fontSize: VG.font.micro, letterSpacing: VG.tracking.wide, color: 'var(--vg-text-3)', textTransform: 'uppercase' }}>
          Loading Hotel
        </div>
      </div>
...
export default function HotelDetailPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--vg-bg)', paddingTop: '60px' }}>
      ...
      {/* Back nav */}
      <div style={{ padding: `1.2rem ${VG.section.x}`, borderBottom: '1px solid var(--vg-border)', background: 'var(--vg-bg-surface)' }}>
        <Link href={`/${locale}/hotels`} style={{
          display: 'inline-flex', alignItems: 'center', gap: '0.5rem',
          textDecoration: 'none', fontFamily: 'var(--font-space-mono)',
          // FIX: 0.46rem → VG.font.micro
          fontSize: VG.font.micro, letterSpacing: VG.tracking.normal,
          textTransform: 'uppercase', color: 'var(--vg-text-3)', transition: VG.transition.color,
        }}
          ...
          <div style={{ position: 'absolute', bottom: '8rem', left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: '0.4rem', zIndex: 3 }}>
            {images.map((_: any, i: number) => (
              <button key={i} onClick={() => setCurrentImg(i)}
                ...
                style={{ width: i === currentImg ? '20px' : '6px', height: '6px', background: i === currentImg ? 'var(--vg-gold)' : 'rgba(255,255,255,0.4)', border: 'none', cursor: 'pointer', transition: 'all 0.3s', padding: 0 }} />
            ))}
          </div>
        </>)}
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', flexWrap: 'wrap' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  {/* FIX: 0.8rem kept — body size, fine */}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 360px', gap: '2rem', padding: `2.5rem ${VG.section.x}`, maxWidth: '1400px', margin: '0 auto' }}
        ...
        <div>
          ...
          <div style={{ background: 'var(--vg-bg-card)', border: '1px solid var(--vg-border)', padding: '2rem', marginBottom: '1px' }}>
            ...
            <p style={{ fontFamily: 'var(--font-dm-sans)', fontSize: VG.font.body, color: 'var(--vg-text-2)', lineHeight: 1.8 }}>
              {hotel.description}
            </p>
          </div>
                ...
                {hotel.roomTypes.map((room: any) => (
                  <div key={room.type} onClick={() => setSelectedRoom(room)}
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <div style={{ fontFamily: 'var(--font-space-mono)', fontSize: VG.font.micro, letterSpacing: VG.tracking.tight, color: 'var(--vg-text-3)' }}>PER NIGHT</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div style={{ background: 'var(--vg-bg-card)', border: '1px solid var(--vg-gold-border)', position: 'sticky', top: '80px' }}>
            ...
            <div style={{ padding: '1.8rem' }}>
              ...
              <div style={{ marginBottom: '1.5rem', paddingBottom: '1.5rem', borderBottom: '1px solid var(--vg-border)' }}>
                ...
                {/* FIX: 0.44rem → VG.font.micro */}
                <span style={{ fontFamily: 'var(--font-space-mono)', fontSize: VG.font.micro, letterSpacing: VG.tracking.tight, color: 'var(--vg-text-3)', marginLeft: '0.5rem' }}>/ NIGHT</span>
              </div>

              {/* Check-in */}
              <div style={{ marginBottom: '1rem' }}>
                <label style={{ ...monoLabel, fontSize: VG.font.micro, display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.5rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '1.5rem' }}>
                ...
                <div>
                  <label style={{ ...monoLabel, fontSize: VG.font.micro, display: 'block', marginBottom: '0.5rem' }}>Rooms</label>
              ...
              {nights > 0 && (
                <div style={{ background: 'var(--vg-bg-surface)', padding: '1rem', marginBottom: '1.2rem' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    {/* FIX: 0.48rem → VG.font.micro */}
                    <span style={{ fontFamily: 'var(--font-space-mono)', fontSize: VG.font.micro, letterSpacing: VG.tracking.tight, color: 'var(--vg-text-2)' }}>TOTAL</span>
                    <span className="vg-stat-num" style={{ fontSize: '1.1rem' }}>{formatCurrency(total, hotel.currency)}</span>
                  </div>
                </div>
              )}

              {/* FIX: Book Now — proper full-width button, clear CTA */}
              <button onClick={handleBooking} className="vg-btn-primary" style={{
                width: '100%', justifyContent: 'center',
                padding: '1.1rem', fontSize: VG.font.button,
