...
export default function WalletPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--vg-bg)', paddingTop: '60px' }}>
      ...
      {/* Header */}
      <div style={{ background: 'var(--vg-bg-surface)', borderBottom: '1px solid var(--vg-border)', padding: 'clamp(2.5rem,5vw,4rem) clamp(1.5rem,7vw,5rem)', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ padding: 'clamp(2rem,5vw,4rem) clamp(1.5rem,7vw,5rem)', maxWidth: '700px', margin: '0 auto' }}>
        ...
        <div style={{ background: 'var(--vg-gold-dim)', border: '1px solid var(--vg-gold-border)', padding: '2rem', marginBottom: '2rem', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ fontFamily: 'var(--font-space-mono)', fontSize: '0.46rem', letterSpacing: '0.25em', textTransform: 'uppercase', color: 'var(--vg-gold)', marginBottom: '1.2rem' }}>Pi Network User</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1.2rem', marginBottom: '1.5rem' }}>
            ...
            <div>
              ...
              <div style={{ fontFamily: 'var(--font-space-mono)', fontSize: '0.44rem', letterSpacing: '0.15em', color: 'var(--vg-text-3)' }}>Pi Network Account</div>
            </div>
          </div>
          ...
          {activePayment && (
            <div style={{ background: 'var(--vg-bg-surface)', border: '1px solid var(--vg-gold-border)', padding: '0.8rem 1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontFamily: 'var(--font-space-mono)', fontSize: '0.44rem', letterSpacing: '0.15em', color: 'var(--vg-gold)' }}>ACTIVE PAYMENT</span>
              <span style={{ fontFamily: 'var(--font-space-mono)', fontSize: '0.72rem', color: 'var(--vg-gold)' }}>π {activePayment.amount}</span>
            </div>
          )}
        </div>
        ...
        <div style={{ background: 'var(--vg-bg-card)', border: '1px solid var(--vg-border)' }}>
          ...
          {paymentHistory.length === 0 ? (
            <div style={{ padding: '4rem 2rem', textAlign: 'center' }}>
              ...
              <a href="/en/hotels" className="vg-btn-outline" style={{ textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              {paymentHistory.map((p) => (
                <div key={p.identifier} style={{ background: 'var(--vg-bg-card)', padding: '1.2rem 1.8rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', transition: 'background 0.2s' }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    ...
                    <div>
                      <div style={{ fontFamily: 'var(--font-dm-sans)', fontSize: '0.86rem', color: 'var(--vg-text)', marginBottom: '0.2rem' }}>{p.memo}</div>
                      <div style={{ fontFamily: 'var(--font-space-mono)', fontSize: '0.42rem', letterSpacing: '0.12em', color: 'var(--vg-text-3)' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <div style={{ fontFamily: 'var(--font-space-mono)', fontSize: '0.4rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: p.status === 'completed' ? '#10b981' : p.status === 'cancelled' ? '#ef4444' : 'var(--vg-gold)', marginTop: '0.15rem' }}>
                      {p.status}
                    </div>
                  </div>
                </div>
