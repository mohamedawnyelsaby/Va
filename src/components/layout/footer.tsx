...
// UPDATED: Fixed all links, added dashboard, improved newsletter
'use client';
import Link from 'next/link';
import { VG, monoLabel } from '@/lib/tokens';

const LINKS = {
  Explore:  [['Hotels','hotels'],['Attractions','attractions'],['Restaurants','restaurants'],['Cities','cities'],['AI Assistant','ai']],
  Account:  [['Dashboard','dashboard'],['My Bookings','bookings'],['Favorites','favorites'],['Wallet','wallet'],['Profile','profile']],
  Company:  [['About','about'],['Blog','blog'],['Careers','careers'],['Press','press'],['Contact','contact']],
  Legal:    [['Privacy','privacy'],['Terms','terms'],['Cookies','cookies'],['Trust & Safety','trust'],['Help Center','help']],
};
    ...
    <footer style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '2.5rem', marginBottom: '3rem' }}>
        ...
        <div>
          ...
          <p style={{ fontFamily: 'var(--font-dm-sans)', fontSize: VG.font.small, color: 'var(--vg-text-2)', lineHeight: 1.7, maxWidth: '220px', margin: '0 0 1.5rem' }}>
            AI-powered global travel platform built on the Pi Network. Book, explore, and earn Pi rewards.
          </p>
          {/* Social */}
          <div style={{ display: 'flex', gap: '0.8rem' }}>
            {[['𝕏','#','X (Twitter)'],['in','#','LinkedIn'],['π','#','Pi Network']].map(([icon, href, label]) => (
              <a key={icon} href={href} aria-label={label}
                style={{ width: '32px', height: '32px', border: '1px solid var(--vg-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--vg-text-2)', textDecoration: 'none', fontSize: '0.75rem', transition: 'color 0.2s, border-color 0.2s' }}
          ...
          <div key={col}>
            ...
            {(items as string[][]).map(([label, slug]) => (
              <Link key={slug} href={`/${locale}/${slug}`}
                style={{ display: 'block', marginBottom: '0.65rem', fontFamily: 'var(--font-dm-sans)', fontSize: VG.font.small, color: 'var(--vg-text-2)', textDecoration: 'none', transition: 'color 0.2s' }}
      ...
      <div style={{ borderTop: '1px solid var(--vg-border)', paddingTop: '1.5rem', display: 'flex', flexWrap: 'wrap', gap: '1rem', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
          ...
          <div className="vg-badge-outline"><span className="dot" />AI-Powered</div>
        </div>
      </div>
    </footer>
  );
