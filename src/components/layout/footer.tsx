...
export function Footer({ locale, isRTL = false }: { locale: string; isRTL?: boolean }) {
  return (
    <footer style={{
      ...
      direction: 'ltr', // always LTR layout for footer
    }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '3rem', marginBottom: '3rem' }}>
        ...
        <div>
          ...
          <p style={{ fontFamily: 'var(--font-dm-sans)', fontSize: '0.78rem', color: 'var(--vg-text-2)', lineHeight: 1.7, maxWidth: '200px', margin: 0 }}>
          ...
          <div style={{ marginTop: '1.5rem', display: 'flex', gap: '0.8rem' }}>
            {[['𝕏','#'],['in','#'],['π','#']].map(([icon, href]) => (
              <a key={icon} href={href} style={{ width: '32px', height: '32px', border: '1px solid var(--vg-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--vg-text-2)', textDecoration: 'none', fontSize: '0.7rem', transition: 'color 0.2s, border-color 0.2s' }}
        ...
        {Object.entries(LINKS).map(([col, items]) => (
          <div key={col} style={{ textAlign: 'left' }}>
            ...
            {(items as string[][]).map(([label, slug]) => (
              <Link key={slug} href={`/${locale}/${slug}`} style={{ display: 'block', marginBottom: '0.65rem', fontFamily: 'var(--font-dm-sans)', fontSize: '0.8rem', color: 'var(--vg-text-2)', textDecoration: 'none', transition: 'color 0.2s', textAlign: 'left' }}
